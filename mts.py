...
import encrypted_manifest_utils as encryptils
import random
...
#gets the filenames of names and sizes steamdb metadata (in json)

def get_metadata_json_filenames(depotid, manifestid, nature):
    if nature = 'names':
     return depotid + '_' + manifestid + '_plain_names.txt' 
    elif nature = 'sizes':
     return depotid + '_' + manifestid + '_plain_sizes.txt'
    else:
     printf("No such nature")
     exit(0)
...
def get_html(depotid, manifestid):
    ...
    #Executes the curl command as a subcommand on the steamdb url of the given depotid
     ...
     os.system("python3 /root/pup.py " + depotid +  " " + manifestid)
...
def scrape_database(depotid, manifestid):
    ...
    raw_html_data = open('/root/f.html', 'rb')
    ...
    plain_names_json_file = get_metadata_json_filenames(depotid, manifestid, 'names')
    plain_sizes_json_file = get_metadata_json_filenames(depotid, manifestid, 'sizes')
    
     #Verifies if the plain names file exists
    
    if not os.path.isfile(plain_names_json_file):
    
     #If not, writes plain names to it    
     
     with open(plain_names_json_file, 'w') as f:
     
      f.write(json.dumps(file_names))
     
     #Verifies if the plain sizes file exists
     
    if not os.path.isfile(plain_sizes_json_file):
     
     #If not, writes plain sizes to it
     
     with open(plain_sizes_json_file, 'w') as f:
...
def deserialize_manifest(depotid, manifestid):
    ...
    manifest_file_sizes = encryptils.get_encrypted_files_sizes(
            data=data)
    
    #Gets manifest file hashs
    
    hashs = encryptils.get_chunks_hashs(data=data)
    
    #Gets base64 serialized manifest file names
    
    encrypted_file_names = encryptils.get_encrypted_filenames_base64(data=data)
...
def organize_encrypted_with_plain_data(depotid, manifestid, output):
    ...
    if not os.path.isfile(plain_names_json_file) or not os.path.isfile(plain_sizes_json_file):
    ...
    plain_db_names = json.loads(open(plain_names_json_file, 'rb').read())
    
    #Parses the json plain sizes list file 
    
    sizes_db = json.loads(open(plain_sizes_json_file, 'rb').read())
    ...
    #Reorder encrypted data with plain data using common sizes (indexes)
    ...
    print(len(sizes_manifest))
    print(len(encrypted_manifest_names))    
 
    #Create dictionnary of data (plain names, sizes, encrypted names)

    encrypted_and_plain_index = {
        'names_plain': plain_db_names,
        'sizes': sizes_c,
        'names_encrypted': encrypted_names
    }
    df = pd.DataFrame(encrypted_and_plain_index)
    df.set_index('names_plain')
    if output == 1:
     filename = 'database' + manifestid
...
if __name__ == '__main__':
    ...
    """parser = argparse.ArgumentParser(description="SDBcrape cli tool")
    parser._action_groups.pop()

    required = parser.add_argument_group('required arguments')
    optional = parser.add_argument_group('

    required.add_argument('-d', '--depot', type=int, default=None)
    optional
    
    print(prepararing_cryptanalysis("731", "7341100646790288242"))"""
    scrape_database('732', '1071977135513850343')
